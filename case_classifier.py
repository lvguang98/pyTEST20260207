...
def build_system_prompt(role: str) -> str:
    """构建 System Prompt —— 角色专属法律规范（文本从 resource/prompts 读取）"""
    role_key = role if role in ("本人", "证人", "法人") else "本人"
    return load_prompt('transcript_system_common') + load_prompt(f'transcript_system_{role_key}')
...
def build_user_prompt(c: CaseClassification,
    ...
    """
    构建 User Prompt —— 传入个案事实（框架文本从 resource/prompts 读取）。
    ...
    accident_section = ""
    if any([a.time, a.location, a.description, a.diagnosis, a.hospital]):
        accident_section = "\n=== 事故信息 ===\n" + "\n".join(line for line in [
            f"事故时间：{a.time}" if a.time else "",
            f"事故地点：{a.location}" if a.location else "",
            f"受伤经过：{a.description[:200]}" if a.description else "",
            f"诊断结论：{a.diagnosis}" if a.diagnosis else "",
            f"送医医院：{a.hospital}" if a.hospital else "",
        ] if line)
    ...
    if c.role != "本人" and person_transcript:
        transcript_section = (
            "\n=== 受伤职工本人笔录（基础事实参考） ===\n"
            f"{person_transcript[:2000]}\n"
            "【重要】以上是受伤职工本人的谈话笔录。请据此：\n"
            "1. 以本人的事实陈述为基础，生成当前角色的提问\n"
            "2. 与本人陈述有出入的地方，追问核实并标注【注意：与本人陈述不一致】\n"
            "3. 本人已陈述的事实无需重复询问，重点补充当前角色特有的信息"
        )
    ...
    if material_summary:
        material_section = (
            "\n=== 材料清单（✓=已提供, ✗=缺失） ===\n"
            f"{material_summary}\n"
            "缺失的材料（✗）必须在谈话中逐项追问：是否持有、如不能提供请说明原因。"
        )

    # ── 用外部模板组装（resource/prompts/谈话笔录用户提示词.txt）──
    prompt = load_prompt('transcript_user')
    prompt = prompt.replace('{{案件信息}}',
                            f"{c.case_nature}，{c.applicant_type}，适用{c.regulation_text}")
    prompt = prompt.replace('{{当前角色}}', c.role)
    prompt = prompt.replace('{{受伤职工}}', c.injured_worker_name)
    prompt = prompt.replace('{{被调查人信息}}', person_section)
    prompt = prompt.replace('{{用人单位信息}}', company_section)
    prompt = prompt.replace('{{事故信息}}', accident_section)
    prompt = prompt.replace('{{案件申请陈述}}', statement_section)
    prompt = prompt.replace('{{本人笔录参考}}', transcript_section)
    prompt = prompt.replace('{{材料清单}}', material_section)
    return prompt
...
def get_output_format_instruction() -> str:
    """获取谈话笔录输出格式指令（文本从 resource/prompts 读取）"""
    return load_prompt('transcript_output_format')
